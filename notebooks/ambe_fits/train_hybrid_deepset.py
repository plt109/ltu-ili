...
class HybridDeepSet(nn.Module):
    """DeepSet embedding with n_events appended.

    global_mlp sees only [mean_pool, max_pool] (2*hidden_channels).
    n_events_norm is appended after, giving NSF input dim: out_channels + 1.
    """
    ...
    def forward(self, x):
        ...
        return torch.cat([deepset_out, n_events_norm], dim=1)
...
def main():
    with open("hybrid_deepset_config.yaml") as f:
    ...
    out_dir = (f"{cfg['save_base']}/"
               f"{cfg['data']['num_samples']}totalsamples_hybrid1_deepset_nsf"
    ...
    # --- Z-norm stats from training set (for DeepSet event normalization) ---
    _train_events = []
