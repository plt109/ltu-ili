...
def load_dataset_from_hdf5(fname, num_samples):
    """Load sims from HDF5, compute [mean(cS1), std(cS1)] per sim."""
    ...
    summaries = np.array(
        [[np.mean(c), np.std(c)] for c in cs1_list], dtype=np.float32
    )
...
def main():
    ...
    parser.add_argument('--config', default='g1_only_config.yaml')
    ...
    # --- NSF (no embedding — 2-dim summary stats straight in) ---
    nets = [
